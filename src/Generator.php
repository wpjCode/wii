...
abstract class Generator extends Model
{
     ...
     * @var string
     */
    private static $assetsPath = 'wpjCode\wii\WiiAsset';
     ...
     */
    abstract public function getName($returnEn = false);
    /**
     * Generates the code based on the current user input and the specified code template files.
     * This is the main method that child classes should implement.
     * Please refer to [[\wpjCode\wii\generators\controller\Generator::generate()]] as an example
    ...
    {
        return $this->langString('code gender success');
    }
    ...
    public function loadStickyAttributes()
    {
        ...
        $stickyAttributes = array_merge($stickyAttributes, [
            'template',
            'jsPath',
            'expName',
            'cssPath',
    ...
    {
        $stickyAttributes = array_merge($this->stickyAttributes(), [
            'template',
            'jsPath',
            'expName',
            'cssPath',
    ...
    {
        return Yii::$app->getRuntimePath() . '/wii-' . Yii::getVersion() . '/' . str_replace('\\', '-', get_class($this)) . '.json';
    }
    ...
    public function render($template, $params = [])
    {
        ...
        $tempFullPath = $this->getTemplatePath() . '/' . $template;
        if (!file_exists($tempFullPath)) {
            $this->addError('template', "模板文件{$template}不存在");
            return false;
        }
        ...
        return $view->renderFile($tempFullPath, $params, $this);
    }
    ...
    {
        $filePath = Yii::getAlias($this->$attribute);

        $fileList = glob($filePath . '.*');
        // 文件是否存在
        if (!$fileList || empty($fileList)) {
            $this->addError($attribute, $this->langString('file no exits'));
            return false;
        }
        return true;
    }
     ...
     * @return string
     */
    public function langString($string = '')
    {

        if (!openI18n) return $string;
        $string = addslashes($string);
        $str = Yii::t(I18nCategory, $string);
        return $str;
    }
    ...
    /**
     * 注册静态资源文件
     * @param yii\web\View $webView
     * @param string $filePath 文件路径
     * @return String|bool
     */
    public static function regAssetsFile($webView, $filePath = '') {

        if (empty($filePath)) return false;

        try {
            $assetsPath = self::getAssetsPath();
            if (!class_exists($assetsPath)) return false;
            $webView->registerAssetBundle($assetsPath);
            $assManger = $webView->getAssetManager();
            $bundle = $assManger->getBundle($assetsPath);
            $jsUrl = $assManger->getAssetUrl($bundle, $filePath);
            $webView->registerJsFile($jsUrl, [
                $assetsPath::className(),
                'depends' => $assetsPath
            ]);
            return true;
