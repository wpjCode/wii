$(function () {
    ...
    // 是否缓存模型点选变化
    $('#generator-cachemodel').on('change', cacheModelChange);

    /**
     * 初始化
     */
    function init() {
        // $('form .field-generator-cachebaseclass').toggle($('#generator-cachemodel').is(':checked'));
    }
    ...
    function randNameModel($dom) {
        ...
        initFunc($dom);
        // 先获取 基础模块 名称
        var modelName = $('#generator-tablename').val();
        // 获取最后名称
     ...
     * 检测缓存模型点选
     */
    function cacheModelChange() {
        $('form .field-generator-cachebaseclass').toggle($(this).is(':checked'));
    }
});
